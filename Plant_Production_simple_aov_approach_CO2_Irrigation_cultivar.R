...
setwd("~/AgFace/Plant_Production/Environment_comparison/CO2xIrrigationxCultivar")
...
# Additionally, select treatments here
# yj <- yj[yj$TOS == "TOS1", ]
...
write.table(yj.sum[yj.sum$variable %in% parameters_to_keep, ],
...
pdf("CO2xIrrigationxCultivar_boxplots_Yipti_Janz_Feb10_free_y_simple.pdf")
        print(my.boxplots)
dev.off()
...
sink("CO2xIrrigationxCultivar_TOS1_Anova_results_after_transformation.txt")
        print(aov.out)
sink()
...
ggsave(file = "CO2xIrrigationxCultivar_ANOVA_overview.pdf", height = 25, width = 16)
