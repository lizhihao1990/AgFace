...
yj <- DCall[(DCall$TrialID == "Walpeup" & DCall$Nitrogen == "N0"), ]
...
write.table(yj.sum,
            file = "CO2xTOS_Yitpi_Walpeup_plant_production_summary_per_year.csv",
            row.names = FALSE,
            na = "",
            sep = ",")
...
write.table(yj.rel.cast,
            file = "CO2xTOS_plant_production_relative_difference_relSD.csv",
            row.names = FALSE,
            na = "",
            sep = ",")
...
pdf("Walpeup_boxplots_relative_response_free_y_simple_aov.pdf")
        print(my.boxplots)
dev.off()
...
pdf("Walpeup_Histograms_for_non_homogeneous_paras.pdf")
        print(my.hist)
dev.off()
...
sink("CO2xTOS_Walpeup_Anova_results_after_transformation.txt")
        print(aov.out)
sink()

my.aov <- aov(trans_value ~ CO2 * TOS,
         data = yj.homogeneous[yj.homogeneous$variable == "Plant.wt..g.plant.",])
summary(my.aov)
str(summary(my.aov))
...
ggsave(file = "CO2xTOS_ANOVA_overview.pdf", height = 21, width = 14)

write.table(aov.details.out,
            file = "CO2xTOS_Walpeup_AOV_pvalues.csv",
            sep = ",", row.names = FALSE)
