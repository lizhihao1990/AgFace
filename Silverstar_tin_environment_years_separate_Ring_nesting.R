...
aov.out <- dlply(df.melt.homogeneous[df.melt.homogeneous$variable %in% parameters_to_keep, ],
                 .(Year, my.Trait, Stage, variable),
                 function(x) {
                 
                 out <- try(summary(aov(value ~ CO2 * Irrigation * Cultivar + 
...
sink("Silverstar_SBx_Year_Anova_results.txt")
        print(aov.out)
sink()
...
sink("Silverstar_SBx_Year_lme_results.txt")
        print(lme.out)
sink()
...
p <- ggplot(relev.p.values,
  ...
  p <- p + facet_grid(Stage ~ my.Trait * Year)
  p <- p + theme_bw()
...
ggsave(file = "Silverstar_SB_p_value_plot.pdf",
       width = 11, height = 19)
