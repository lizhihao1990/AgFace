...
setwd("~/AgFace/Plant_Production/Environment_comparison/CO2xTOSxCultivar")
...
pdf("CO2xTOSxCultivar_boxplots_Yipti_Janz_Feb10_free_y_simple.pdf")
        print(my.boxplots)
dev.off()
...
lev.out <- leveneTest(Yield..g.m2. ~ CO2 * TOS * Cultivar, 
                      data = yj)
lev.out$`Pr(>F)`[1]
...
levene.out <- dlply(yj.melt,
                 ...
                 out <- try(leveneTest(value ~ CO2 * TOS * Cultivar,
                                    data = x))
...
# out <- BoxcoxTrans(yj.test, yj.test$value, yj.test$CO2, yj.test$TOS, yj$Cultivar)

# data frame with the transformed data
yj.transformed  <- ddply(yj.to_transform,
                    .(TrialID, Year, variable, Stage),
                    function(x){
                    BoxcoxTrans(x, x$value, x$CO2, x$TOS, x$Cultivar)
                    })
...
after.tests <- ddply(yj.transformed,
                 ...
                 out <- try(leveneTest(trans_value ~ CO2 * TOS * Cultivar,
                                    data = x))
...
aov.out <- dlply(yj.homogeneous[yj.homogeneous$variable %in% parameters_to_keep, ],
                 ...
                 out <- try(summary(aov(trans_value ~ CO2 * TOS * Cultivar,
                                    data = x,
                                    na.action = na.omit)))
...
sink("CO2xTOSxCultivar_Anova_results_after_transformation.txt")
        print(aov.out)
sink()

my.aov <- aov(trans_value ~ CO2 * TOS * Cultivar,
...
ggsave(file = "CO2xTOSxCultivar_ANOVA_overview.pdf", height = 25, width = 16)
