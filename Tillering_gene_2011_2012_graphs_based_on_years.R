...
ggsave(file = "Fig.01.Tillering_se_per_Year.pdf", 
...
ggsave(file = "Fig.02.Yield_se_per_Year.pdf", 
       width = my.width, height = my.height, 
       units = "cm")
...
ggsave(file = "Grains_per_tiller_se_per_Year.pdf", 
       width = my.width, height = my.height, 
       units = "cm")
...
ggsave(file = "Percent_N_grains_se_per_Year.pdf", 
       width = my.width, height = my.height, 
       units = "cm")
...
ggsave(file = "Percent_N_plant_se_per_Year.pdf", 
       width = my.width, height = my.height, 
       units = "cm")
...
p <- ggplot(df[df$Stage != "DC31", ], 
  ...
  p <- p + facet_grid(Stage ~ Year * Irrigation)
  p <- p + theme_my
  p <- p + theme(legend.position = c(0.38, 0.9))
p

fig.above.root.biomass <- p

ggsave(file = "Above_root_biomass_se_per_Year.pdf", 
       width = my.width, height = my.height, 
       units = "cm")
...
pdf(file = "Tillering_figures_per_year.pdf",
    width = my.width/2.54, height = my.height/2.54)
    print(my.figures.list)
dev.off()

pdf(file = "Carbohydrates_leaf_stem_figures_per_year.pdf",
    width = my.width/2.54, height = my.height/2.54)
...
dim(tin)
write.table(tin, file = "Tin_plantprod_and_Carbs.csv", row.names= F, sep = ",")

save(tin, file = "Tin_2011_2012_plant_production_Carbs.RData", compress = TRUE)
