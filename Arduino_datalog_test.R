...
df <- read.csv("~/AgFace/2015/Arduino/logger_test/DATALOG_SLD_Q190.CSV", header = FALSE)

#names(df) <- c("Epoch", "Date", "Temp",	"rH", "PARmVA",	"PARmVB", "Batt_mV", "TDRmV")
names(df) <- c("Epoch", "Date", "LightmV", "LicormV", "Batt")
df$Date <- as.POSIXct(df$Date,
             format = "%Y-%m-%d %H:%M:%S")
# get rid of data when there was no Batt_mV
start.time <- as.POSIXct("2015-08-28 14:40:00")
df <- df[df$Date > start.time, ]

df$LicormV[df$LicormV > 4000] <- NA
...
p <- ggplot(df.melt,#[df.melt$variable == "LightmV", ],
            aes(x = Date, y = value))
