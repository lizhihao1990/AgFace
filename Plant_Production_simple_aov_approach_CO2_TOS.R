...
setwd("~/AgFace/Plant_Production/Environment_comparison/CO2xTOS")
...
yj <- yj[yj$Irrigation == "Rain", ]

# And for this AOV configuration, only Walpeup is used.
yj <- yj[yj$TrialID == "Walpeup", ]
...
write.table(yj.sum,
            file = "CO2xTOS_Yitpi_Walpeup_only_plant_production_summary_per_year.csv",
            row.names = FALSE,
            na = "",
            sep = ",")
...
my.boxplots <- dlply(yj.melt[yj.melt$variable %in% parameters_to_keep, ],
                  ...
                  MyBoxplot(dataframe = x, 
                            ...
                            facet_var_b = "Year",
                            two_separators = TRUE,
                            the_x_label = "Time of sowing",
                            the_main_title = ""
                            )})

# orig line: MyBoxplot(x, x$variable, "Ord.Environment", "value", "CO2", "CO2", "Stage", ".")})

pdf("CO2xTOS_Yitpi_Walpeup_onlyboxplots_simple.pdf")
        print(my.boxplots)
dev.off()
...
yj.rel.cast <- cast(yj.rel.melt,
                    TrialID + Year + CO2 + TOS + Stage + Cultivar ~ parameter + variable,
                    fun = mean)

write.table(yj.rel.cast,
            file = "CO2xTOS_plant_production_relative_difference_per_environment_relSD.csv",
            row.names = FALSE,
            na = "",
            sep = ",")
...
lev.out <- leveneTest(Yield..g.m2. ~ CO2 * TOS, 
                      data = yj)
lev.out$`Pr(>F)`[1]
...
levene.out <- dlply(yj.melt,
                 ...
                 out <- try(leveneTest(value ~ CO2 * TOS,
                                    data = x))
...
yj.to_transform <- yj.melt[
                  interaction(yj.melt$variable, yj.melt$Stage) %in% 
...
yj.test <- yj.to_transform[yj.to_transform$variable == "Plant.wt..g.plant.", ]
...
out <- BoxcoxTrans(yj.test, yj.test$value, yj.test$CO2, yj.test$TOS)

# data frame with the transformed data
yj.transformed  <- ddply(yj.to_transform,
                    .(TrialID, Year, variable, Stage),
                    function(x){
                    BoxcoxTrans(x, x$value, x$CO2, x$TOS)
                    })
...
after.tests <- ddply(yj.transformed,
                 ...
                 out <- try(leveneTest(trans_value ~ CO2 * TOS,
                                    data = x))
...
aov.out <- dlply(yj.homogeneous[yj.homogeneous$variable %in% parameters_to_keep, ],
                 ...
                 out <- try(summary(aov(trans_value ~ CO2 * TOS,
                                    data = x,
                                    na.action = na.omit)))
...
sink("CO2xTOS_Yitpi_Walpeup_only_Anova_results_after_transformation.txt")
        print(aov.out)
sink()

my.aov <- aov(trans_value ~ CO2 * TOS,
         data = yj.homogeneous[yj.homogeneous$variable == "Yield..g.m2.",])
summary(my.aov)
str(summary(my.aov))
summary(my.aov)[[1]][["Pr(>F)"]]

#aov.details.out <- ddply(yj.homogeneous,
aov.details.out <- ddply(yj.homogeneous[yj.homogeneous$variable %in% parameters_to_keep, ], 
                 ...
                 if (inherits(out, "try-error")) {
                      print("problem with fit")
                      aov.out <- c(rep(NA, 3))
                      names(aov.out) <- c("CO2", "TOS", "CO2:TOS")
                      return(aov.out)} 
                 else {
                      print("successful fit")
                      my.terms <- rownames(summary(out)[[1]])[1:3]
                      print(my.terms)
                      aov.out <- summary(out)[[1]][["Pr(>F)"]][1:3]
                      print(aov.out)
...
p <- ggplot(aov.res, aes(x = variable, y = Parameter))
  p <- p + geom_point(aes(colour = value, shape =TrialID), 
                      size = 3.5, alpha = 0.66, 
                      position = position_dodge(width = 0.1, height = 0))
...
ggsave(file = "CO2xTOS_Yitpi_Walpeup_only_ANOVA_overview.pdf", height = 25, width = 16)
