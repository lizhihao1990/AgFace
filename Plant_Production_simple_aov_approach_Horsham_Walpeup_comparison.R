...
load("../Plant_Production_2007_2009_Glenn_Oct24_2013.RData")
...
yj.sum <- cast(yj.melt, 
             TrialID + CO2 + Stage + Year + Cultivar ~ variable, 
...
pdf("Horsham_Walpeup_Environments_boxplots_Yipti_Janz_Oct24_free_y_simple.pdf")
        print(my.boxplots)
dev.off()
...
yj.rel.melt <- melt(yj.rel,
                    id = names(yj.rel)[1:13])

yj.rel.cast <- cast(yj.rel.melt,
                    TrialID + Year + CO2 + Irrigation + Stage + Cultivar ~ parameter + variable,
                    fun = mean)
...
yj.transformed  <- ddply(yj.to_transform,
                    .(TrialID, Year, variable, Stage),
                    function(x){
                    BoxcoxTrans(x, x$value, x$CO2, x$TOS, x$Irrigation)
                    })
...
aov.out <- dlply(yj.homogeneous,
                 ...
                 out <- try(summary(aov(trans_value ~ CO2 * TOS * Irrigation * Cultivar,
                                    data = x,
                                    na.action = na.omit)))
...
my.aov <- aov(trans_value ~ CO2 * TOS * Irrigation * Cultivar,
...
aov.details.out <- ddply(yj.homogeneous,
                 ...
                 out <- try(aov(
                       trans_value ~ CO2 * TOS * Irrigation * Cultivar,
                                    data = x,
                                    na.action = na.omit))
                 ...
                 if (inherits(out, "try-error")) {
                      ...
                      print("successful fit")
                      my.terms <- rownames(summary(out)[[1]])[1:15]
                      aov.out <- summary(out)[[1]][["Pr(>F)"]][1:15]
...
p <- ggplot(aov.res, aes(x = variable, y = Parameter))
  p <- p + geom_point(aes(colour = value, shape = TrialID), size = 5)
...
ggsave(file = "ANOVA_overview.pdf", height = 25, width = 16)

aov.res[aov.res$variable == "CO2:Cultivar", ]
