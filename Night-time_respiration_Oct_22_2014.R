...
setwd("/home/loewi/AgFace/2014/Night_time_respiration/2014-10-22")

# import the data files, one file per instrument
PSC0161 <- read.csv("2014-10-22_nighttime_respiration_0161.csv",
                    skip = 9)
PSC1256 <- read.csv("2014-10-22_nighttime_respiration_1256.csv",
                    skip = 12, sep = "\t")
PSC1375 <- read.csv("2014-10-22_nighttime_respiration_1375.csv",
                    skip = 14, sep = "\t")
...
nightres2$bad.data[nightres2$Cultivar == "Yitpi" &
                  nightres2$Ring == 6 &
                  nightres2$Comment == "wet 1"] <- "bad"

nightres3 <- nightres2[nightres2$bad.data == "good", ]

nightres3$bad.data[nightres3$Photo > 0.5] <- "bad"
...
p <- ggplot(nightres, aes(x = Cultivar, y = Photo))
     p <- p + geom_boxplot(aes(colour = as.factor(Leaf), fill = Treatment))
     p <- p + facet_grid(. ~ Ring)
p
...
ggsave(file = "2014-10-22_Nighttime_respiration.pdf",
       width = 9, height = 7)
